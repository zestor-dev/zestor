...
func TestGetAll(t *testing.T) {
	...
	if _, err := s.Set("a", "k1", TestData{Name: "x", Value: 1}); err != nil {
		t.Fatal(err)
	}
	if _, err := s.Set("b", "k2", TestData{Name: "y", Value: 2}); err != nil {
		t.Fatal(err)
	}
	...
	if len(all["a"]) != 1 || len(all["b"]) != 1 {
		t.Fatalf("GetAll() unexpected layout: %#v", all)
	}
	if all["a"]["k1"].Value != 1 || all["b"]["k2"].Value != 2 {
...
func TestWatchUpdate(t *testing.T) {
	...
	go func() {
		...
		_, _ = s.Set(kind, key, val2)
	}()

	select {
	case ev := <-ch:
		if ev.EventType != store.EventTypeUpdate {
			t.Errorf("Event type = %s, want %s", ev.EventType, store.EventTypeUpdate)
		}
		if ev.Object.Name != val2.Name || ev.Object.Value != val2.Value {
			t.Errorf("Event object = %v, want %v", ev.Object, val2)
		}
	case <-time.After(10 * time.Second):
...
func TestWatchDelete(t *testing.T) {
	...
	case ev := <-ch:
		if ev.EventType != store.EventTypeDelete {
			t.Errorf("Event type = %s, want %s", ev.EventType, store.EventTypeDelete)
		}
		if ev.Object.Name != val.Name || ev.Object.Value != val.Value {
			t.Errorf("Event object = %v, want %v", ev.Object, val)
		}
...
func TestNamespaceIsolation(t *testing.T) {
	...
	kind := "test"
	key := "shared_key"
	val1 := TestData{Name: "ns1", Value: 1}
	val2 := TestData{Name: "ns2", Value: 2}

	_, err = s1.Set(kind, key, val1)
	if err != nil {
		t.Fatalf("s1.Set() error = %v", err)
	}

	_, err = s2.Set(kind, key, val2)
	if err != nil {
		t.Fatalf("s2.Set() error = %v", err)
	}

	got1, ok1, _ := s1.Get(kind, key)
	got2, ok2, _ := s2.Get(kind, key)

	if !ok1 || !ok2 {
		t.Fatal("Failed to get values from both namespaces")
	}

	if got1.Name != "ns1" || got1.Value != 1 {
		t.Errorf("s1.Get() = %v, want %v", got1, val1)
	}

	if got2.Name != "ns2" || got2.Value != 2 {
		t.Errorf("s2.Get() = %v, want %v", got2, val2)
	}
}
...
func TestConcurrentAccess(t *testing.T) {
	...
	_, err := s.Set(kind, key, TestData{Name: "counter", Value: 0})
	if err != nil {
		t.Fatalf("Set() error = %v", err)
	}
	...
	if got.Value != expected {
		t.Errorf("Final value = %d, want %d", got.Value, expected)
	}
}
...
func BenchmarkSetFn(b *testing.B) {
	...
	kind := "bench"
	key := "counter"
	_, _ = s.Set(kind, key, TestData{Name: "counter", Value: 0})
